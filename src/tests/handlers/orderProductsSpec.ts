...
describe('Tests endpoint responses for /order_products', () => {
  ...
  it('should get order_products', async () => {
    const response = await request.get('/order_products').set('Authorization', 'bearer ' + jwtToken);
    expect(response.status).toBe(200);
  });

  it('should get order_product with ID', async () => {
    const response = await request.get('/order_products/1').set('Authorization', 'bearer ' + jwtToken);
    expect(response.status).toBe(200);
  });
  ...
  it('should delete order_product with token', async () => {
    const response = await request.delete('/order_products/100').set('Authorization', 'bearer ' + jwtToken);
    expect(response.status).toBe(200);
  });
});
