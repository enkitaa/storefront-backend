...
export class OrderProductList {
  ...
  async show(id: number): Promise<OrderProduct> {
    try {
      ...
      throw new Error(`Could not find ordered product with id ${id}. Error: ${err}`);
    }
  }
  async create(order: OrderProduct): Promise<OrderProduct> {
    try {
      ...
      throw new Error(`Could not add order ${order.order_id} for product ${order.product_id}. Error: ${err}`);
    }
  }
  ...
  async update(id: number, o: OrderProduct): Promise<OrderProduct> {
    try {
      ...
      throw new Error(`Could not update order product ${o.order_id}. Error: ${err}`);
    }
  }
}
